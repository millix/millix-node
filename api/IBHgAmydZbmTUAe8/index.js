...
class _IBHgAmydZbmTUAe8 extends Endpoint {
    ...
    handler(app, req, res) {
        ...
        database.firstShardZeroORShardRepository('transaction', req.query.p1, transactionRepository => {
            ...
            transaction['transaction_child_list']     = [];
            ...
            data.forEach(row => {
                if (!signatures.has(row.address_base)) {
                    ...
                    transaction['transaction_signature_list'].push(_.pick(row, 'address_base', 'signature'));
                }

                if (!inputs.has(row.input_position)) {
                    inputs.add(row.input_position);
                    transaction['transaction_input_list'].push(_.mapKeys(_.pick(row, 'input_position', 'output_transaction_id', 'output_shard_id', 'output_position', 'output_transaction_date', 'input_double_spend_date', 'input_is_double_spend', 'input_address', 'input_address_key_identifier'), (v, k) => inputKeyMap[k] ? inputKeyMap[k] : k));
                }

                if (!outputs.has(row.output_position)) {
                    outputs.add(row.input_position);
                    transaction['transaction_output_list'].push(_.mapKeys(_.pick(row, 'output_position', 'output_address', 'output_address_key_identifier', 'amount', 'output_stable_date', 'output_is_stable', 'spent_date', 'is_spent', 'output_double_spend_date', 'output_is_double_spend'), (v, k) => outputKeyMap[k] ? outputKeyMap[k] : k));
                }

                if (row.transaction_id_parent && !parents.has(row.transaction_id_parent)) {
                    parents.add(row.transaction_id_parent);
                    transaction['transaction_parent_list'].push(row.transaction_id_parent);
                }

                if (row.transaction_id_child && !childen.has(row.transaction_id_child)) {
                    childen.add(row.transaction_id_child);
                    transaction['transaction_child_list'].push(row.transaction_id_child);
                }

                if (row.audit_point_id && !auditPoints.has(row.audit_point_id)) {
                    auditPoints.add(row.audit_point_id);
                    transaction['transaction_audit_list'].push(row.audit_point_id);
                }
